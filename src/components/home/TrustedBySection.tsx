 ...
 * Doubled list keeps the loop seamless.
 */
      ...
      <div className="mt-6 overflow-hidden [mask-image:linear-gradient(90deg,transparent,black_15%,black_85%,transparent)]">
        <div className="flex w-max animate-marquee gap-16 px-8">
          {[...logos, ...logos].map((logo, i) => (
            <span
              key={`${logo}-${i}`}
              className="font-display text-lg font-bold tracking-[0.2em] text-muted-foreground/60 transition-colors hover:text-primary"
            >
              {logo}
            </span>
          ))}
