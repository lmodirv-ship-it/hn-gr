  ...
  return (
    <div className="space-y-6">
      <header>
        ...
        <h1 className="mt-1 font-display text-3xl font-bold sm:text-4xl">{tt("activity.title")}</h1>
        <p className="mt-2 text-sm text-muted-foreground">
          {tt("activity.subtitle")}
        </p>
      </header>
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder={tt("activity.search")}
      ...
      {filtered.length === 0 ? (
        <div className="grid place-items-center rounded-2xl border border-dashed border-border py-16">
          ...
          <p className="text-sm text-muted-foreground">{tt("activity.empty")}</p>
        </div>
      ) : (
        <div className="overflow-hidden rounded-2xl border border-border">
          <table className="w-full text-sm">
            <thead className="bg-surface/60 text-left text-xs uppercase tracking-wider text-muted-foreground">
              <tr>
                <th className="px-4 py-3">{tt("activity.when")}</th>
                <th className="px-4 py-3">{tt("activity.actor")}</th>
                <th className="px-4 py-3">{tt("common.action")}</th>
                <th className="px-4 py-3">{tt("activity.target")}</th>
            ...
            <tbody className="divide-y divide-border">
              {filtered.map((l) => (
                <tr key={l.id} className="hover:bg-surface/30">
                  ...
                  <td className="px-4 py-2.5">{l.actor_email ?? tt("common.system")}</td>
